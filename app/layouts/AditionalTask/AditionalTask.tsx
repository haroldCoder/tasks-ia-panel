...
import { assignAditionalTaskThunk } from "@/app/shared/slices/tasks/tasksSlice";
...
export const AditionalTask = ({
  ...
  const { loadingAssignAditional } = useSelector((state: RootState) => state.tasks);
  ...
  const assignDataAditionalTask = async () => {
    ...
    await dispatch(assignAditionalTaskThunk({
      id,
    ...
    <>
      {
        (warning || loadingAssignAditional) && (
          <Modal className={!loadingAssignAditional ? style.modal : style.no_modal} 
          ...
          onClose={() => setWarning(false)}>
            {
              ...
              <p id="text-spc">- back to select in calendar, 
                for remove dates selecteds and 
                select first calendar after second calendar</p>
            </div>
              ) : (
